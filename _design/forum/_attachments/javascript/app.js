...
$(function() {
	...
	refreshThreads = function(callback) {
		$db.view('forum/threads_by_forum', {
			descending: true,
			startkey: [currentForum],
	...
	modern_forum = $.sammy('#page_frame', function() {
		...
		if (BrowserDetection.is_mobile) {
			...
			forumApp.bind('load-thread-list', function(ev, params) {
				refreshForums(function(response) {
					...
					var tpl = Locale.getTranslation('forums.ms', locale);
	        ...
	        refreshThreads(function(response) {
      	    ...
      	    var template2 = Locale.getTranslation('threads.ms', locale);
        	  tmp2 = $.mustache(template2, data);
          	//secondaryFrame.append(tmp2);
          	secondaryFrame.html(tmp+tmp2);
					});
				});
			});
		}

		this.before({only: { verb: 'get' }, expect: { path: ['#/', '#/forum/all'] }}, function(context) {
			context.trigger('load-thread-list');
		});

		this.get('#/', function(context) {
			context.redirect('#',  'forum', 'all', '1');
		});
      ...
      context.redirect('#/');
		...
		this.get('#/forum', function(context) {
			context.redirect('#', 'forum', 'all', '1');
		});

		this.get('#/forum/:id', function(context) {
			var forum_id = this.params.id;
			context.redirect('#', 'forum', forum_id , '1');
		});


		this.get('#/forum/:id/:page', function(context) {
			var forum_id = this.params.id;
			if(forum_id!='all')	currentForum = forum_id;
			refreshForums(function(response) {
				var data = {};
				data.forums = response.rows;
...
*/
		...
		this.get("#/user/:id", function(context) {
			var uname = this.params.id
			fetchUserProfile([uname], function(response) {
				var data = {};
				data.user = response.rows[0].value;
				data.user.id = response.rows[0].id;
				var template = context.get_trans('user_profile.ms', locale);
        var tmp = context.interpolate(template, data, 'ms');
		...
		this.put("#/post/pm", function(context) {
			var postPM = this.params.postPM;
			var to_id = this.params.receiver_id;
			var to_name = this.params.receiver_name;
			var pmTitle = this.params.pmTitle;
			var pmDoc;
			var today = new Date(), now = today.getTime();
			if( to_name == username)	{
				alert('You don\'t need to send private message to yourself.');
				context.redirect('#/');
			}
		...
		this.get('#/pm', function(response) {
			var fetchpm = function(callback) {
				$db.view('forum/pm_listing', {
					key: [username],
					descending: true,
					success: callback
				});
			...
			fetchpm(function(response)	{
				var data= {};
				var template =  Locale.getTranslation('view_pm_frame.ms', locale);
				var tmp = $.mustache(template, {
					'pm' : $.map(response.rows, function(pm, index) {
							return {
								'title' : pm.value.title,
								'user_id' : pm.value.user_id,
								'receiver_name' : pm.value.receiver_name,
								'content': pm.value.content
							};
						})
					});
